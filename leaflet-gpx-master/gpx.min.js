...
var L = L || require("leaflet"), _MAX_POINT_INTERVAL_MS = 15e3, _SECOND_IN_MILLIS = 1e3,
    _MINUTE_IN_MILLIS = 60 * _SECOND_IN_MILLIS, _HOUR_IN_MILLIS = 60 * _MINUTE_IN_MILLIS,
    _DAY_IN_MILLIS = 24 * _HOUR_IN_MILLIS, _GPX_STYLE_NS = "http://www.topografix.com/GPX/gpx_style/0/2",
    _DEFAULT_MARKER_OPTS = {
        ...
        shadowAnchor: [16, 47],
        clickable: !1
    }, _DEFAULT_POLYLINE_OPTS = {color: "blue"}, _DEFAULT_GPX_OPTS = {parseElements: ["track", "route", "waypoint"]};
L.GPX = L.FeatureGroup.extend({
    initialize: function (t, e) {
        e.max_point_interval = e.max_point_interval || _MAX_POINT_INTERVAL_MS, e.marker_options = this._merge_objs(_DEFAULT_MARKER_OPTS, e.marker_options || {}), e.polyline_options = e.polyline_options || {}, e.gpx_options = this._merge_objs(_DEFAULT_GPX_OPTS, e.gpx_options || {}), L.Util.setOptions(this, e), L.GPXTrackIcon = L.Icon.extend({options: e.marker_options}), this._gpx = t, this._layers = {}, this._init_info(), t && this._parse(t, e, this.options.async)
        ...
        var i = this, o = function (t, e) {
            ...
            n && (i.addLayer(n), i.fire("loaded", {layers: n, element: t}))
        };
        if ("<" === t.substr(0, 1)) {
            var a = new DOMParser;
        ...
        var n, i, o, a, _ = [], r = [], s = e.gpx_options.parseElements;
        -1 < s.indexOf("route") && r.push(["rte", "rtept"]), -1 < s.indexOf("track") && r.push(["trkseg", "trkpt"]), 0 < (u = t.getElementsByTagName("name")).length && (this._info.name = u[0].textContent), 0 < (f = t.getElementsByTagName("desc")).length && (this._info.desc = f[0].textContent);
        var l = t.getElementsByTagName("author");
        0 < l.length && (this._info.author = l[0].textContent);
        var m = t.getElementsByTagName("copyright");
        for (0 < m.length && (this._info.copyright = m[0].textContent), i = 0; i < r.length; i++) for (a = t.getElementsByTagName(r[i][0]), n = 0; n < a.length; n++) {
            var h = this._parse_trkseg(a[n], e, r[i][1]);
            for (o = 0; o < h.length; o++) _.push(h[o])
        }
        if (this._info.hr.avg = Math.round(this._info.hr._total / this._info.hr._points.length), this._info.cad.avg = Math.round(this._info.cad._total / this._info.cad._points.length), this._info.atemp.avg = Math.round(this._info.atemp._total / this._info.atemp._points.length), -1 < s.indexOf("waypoint")) for (a = t.getElementsByTagName("wpt"), n = 0; n < a.length; n++) {
            var p = new L.LatLng(a[n].getAttribute("lat"), a[n].getAttribute("lon")),
                g = a[n].getElementsByTagName("name"), u = "";
            0 < g.length && (u = g[0].textContent);
            var c = a[n].getElementsByTagName("desc"), f = "";
            0 < c.length && (f = c[0].textContent);
            var d = a[n].getElementsByTagName("sym"), v = "";
            0 < d.length && (v = d[0].textContent);
            var I, x = e.marker_options.wptIcons, M = e.marker_options.wptIconUrls;
            if (x && x[v]) I = x[v]; else if (M && M[v]) I = new L.GPXTrackIcon({iconUrl: M[v]}); else if (x && x[""]) I = x[""]; else {
                if (!M || !M[""]) {
                    console.log('No icon or icon URL configured for symbol type "' + v + '", and no fallback configured; ignoring waypoint.');
                    continue
                }
                I = new L.GPXTrackIcon({iconUrl: M[""]})
            }
            var k = new L.Marker(p, {clickable: e.marker_options.clickable, title: u, icon: I});
        ...
        for (var o = [], a = [], _ = [], r = null, s = 0; s < i.length; s++) {
            ...
            if (l.meta = {
                ...
                cad: null,
                atemp: null
            }, 0 < (u = i[s].getElementsByTagName("time")).length ? l.meta.time = new Date(Date.parse(u[0].textContent)) : l.meta.time = new Date("1970-01-01T00:00:00"), 0 < (u = i[s].getElementsByTagName("ele")).length && (l.meta.ele = parseFloat(u[0].textContent)), 0 < (u = i[s].getElementsByTagName("name")).length) for (var m = u[0].textContent, h = e.marker_options.pointMatchers || [], p = 0; p < h.length; p++) if (h[p].regex.test(m)) {
                a.push({label: m, coords: l, icon: h[p].icon, element: i[s]});
                break
            }
        ...
        var u, c = this._merge_objs(_DEFAULT_POLYLINE_OPTS, {}), f = t.getElementsByTagNameNS(_GPX_STYLE_NS, "line");
        ...
        if (this.fire("addline", {
            ...
            var v = new L.Marker(o[0], {
                ...
                icon: e.marker_options.startIcon || new L.GPXTrackIcon({iconUrl: e.marker_options.startIconUrl})
            });
            this.fire("addpoint", {point: v, point_type: "start", element: i[0]}), _.push(v)
        }
        if (e.marker_options.endIcon || e.marker_options.endIconUrl) {
            v = new L.Marker(o[o.length - 1], {
                clickable: e.marker_options.clickable,
                icon: e.marker_options.endIcon || new L.GPXTrackIcon({iconUrl: e.marker_options.endIconUrl})
            });
            this.fire("addpoint", {point: v, point_type: "end", element: i[i.length - 1]}), _.push(v)
        }
        for (s = 0; s < a.length; s++) {
            v = new L.Marker(a[s].coords, {clickable: e.marker_options.clickable, title: a[s].label, icon: a[s].icon});
            this.fire("addpoint", {point: v, point_type: "label", element: a[s].element}), _.push(v)
        }
        return _
    }, _dist2d: function (t, e) {
        var n = this._deg2rad(e.lat - t.lat), i = this._deg2rad(e.lng - t.lng),
        ...
        var n = this._dist2d(t, e), i = Math.abs(e.meta.ele - t.meta.ele);
